...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        self.w_qs = nn.Linear(2 * d_model, n_head * d_k, bias = False)
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        mb_size, d_model = q.size()   #对于NELL 这里d_model=200，是为了 qkv的维度刚刚好等于200/8=25
        mb_size, d_model = k.size()
        mb_size, d_model = v.size()

        q_s = self.w_qs(q).view(mb_size, -1, d_k)
        k_s = self.w_ks(k).view(mb_size, -1, d_k)
        v_s = self.w_vs(v).view(mb_size, -1, d_v)
        ...
        # 这里的多头采用叠加和
        outputs = torch.sum(outputs, dim=1)/self.n_head
...
class SupportEncoder(nn.Module):
    ...
    def __init__(self, d_model, d_inner, dropout=0.1):
        ...
        init.xavier_normal_(self.proj1.weight)    #映射层参数初始化
    ...
    def forward(self, x):
        ...
        output = self.relu(self.proj1(self.dropout(x)))
...
class ContextAwareEncoder(nn.Module):
    """Use self-attention here  上下文感知编码器（多头注意力）"""
